...
class MyMamba(nn.Module):
    def __init__(self):
        ...
        self.Convs_piror = nn.ModuleList([
            BasicConv(6, base_channel * 2, kernel_size=3, relu=True, stride=1),
            BasicConv(6, base_channel * 2 * 2, kernel_size=1, relu=True, stride=1),
            BasicConv(6, base_channel * 4 * 2, kernel_size=1, relu=True, stride=1),
            BasicConv(6, base_channel * 8 * 2, kernel_size=1, relu=True, stride=1),
            BasicConv(6, base_channel * 4 * 2, kernel_size=1, relu=True, stride=1),
            BasicConv(6, base_channel * 2 * 2, kernel_size=1, relu=True, stride=1),
            BasicConv(6, base_channel * 2, kernel_size=3, relu=True, stride=1),
        ])
    ...
    def forward(self, x, x_peak):

        x_p = torch.cat((x, x_peak), dim=1)
...
if __name__ == '__main__':
    ...
    x_peak = torch.randn((1, 3, 256 * 256)).cuda()
    out = model(x, x_peak)
    print(out.shape)
